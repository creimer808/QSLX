...
export function ContactEditForm({ contactId, onClose }: ContactEditFormProps) {
  const utils = api.useUtils();
  const { data: contact } = api.contact.getById.useQuery({ id: contactId });
  ...
  // Populate form with contact data when it's available
  useEffect(() => {
    if (contact) {
      setFormData({
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div className="w-full max-w-2xl rounded-lg bg-white p-6 shadow-xl">
          <p className="text-center text-gray-500">Loading contact...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="relative w-full max-w-2xl rounded-lg bg-white p-6 shadow-xl">
        ...
        <h2 className="mb-4 text-2xl font-bold text-gray-900">Edit Contact</h2>
